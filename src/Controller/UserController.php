...
{ 
    ...
    public function apiSearch(Request $request, UserRepository $userRepository, PaginatorInterface $paginator): JsonResponse
    {
        $criteria = $request->query->get('criteria');
        $value = $request->query->get('value');
        $page = $request->query->getInt('page', 1);
        $limit = $request->query->getInt('limit', 5);
        $sortCriteria = $request->query->get('sortCriteria');
        $sortDirection = $request->query->get('sortDirection', 'asc');

        $queryBuilder = $userRepository->createQueryBuilder('u');
        
        // Set default ordering if no sort criteria provided
        if ($sortCriteria) {
            // Map the frontend sort criteria to database fields
            $sortField = match($sortCriteria) {
                'username' => 'u.user_username',
                'firstname' => 'u.user_firstname',
                'lastname' => 'u.user_lastname',
                default => 'u.user_username'
            };
            $queryBuilder->orderBy($sortField, $sortDirection === 'asc' ? 'ASC' : 'DESC');
        } else {
            $queryBuilder->orderBy('u.user_username', 'ASC');
        }

        if ($criteria && $value) {
            switch ($criteria) {
                case 'username':
                    $queryBuilder->andWhere('u.user_username LIKE :value')
                        ->setParameter('value', '%' . $value . '%');
                    break;
                case 'firstname':
                    $queryBuilder->andWhere('u.user_firstname LIKE :value')
                        ->setParameter('value', '%' . $value . '%');
                    break;
                case 'lastname':
                    $queryBuilder->andWhere('u.user_lastname LIKE :value')
                        ->setParameter('value', '%' . $value . '%');
                    break;
            }
        }
        ...
        return $this->json([
            ...
            'pageCount' => ceil($pagination->getTotalItemCount() / $limit),
            'currentPage' => $page,
            'itemsPerPage' => $limit
        ]);
    }
    ...
    public function exportPdf(UserRepository $userRepository): Response
    {
        ...
        $options->set('chroot', $publicDirectory);
